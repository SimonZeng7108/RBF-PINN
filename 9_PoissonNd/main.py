...
dim_in = 3 #input shape; e.g. high dimension input x, 5 in this case
...
#Initialise DataSet
dataset = Geo_Dataset(dim_in)
bc_x, bc_u, col_x = dataset()


#Initialise DataLoader
col_loader = DataLoader(Col_Data(col_x), batch_size=[len(col_x) if not Batch_Learning else Batchsize][0], shuffle=True)
bc_loader = DataLoader(BC_Data(bc_x, bc_u), batch_size=[len(bc_x) if not Batch_Learning else Batchsize][0], shuffle=True)
