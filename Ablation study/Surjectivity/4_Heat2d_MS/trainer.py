...
class Trainer:
    ...
    def IC_loss(self, pred, target):
        pred_u = pred[:, 0:1]
        target_u = target[:, 0:1]
        return self.MSE_loss(pred_u, target_u)
    ...
    def PDE_loss(self, pred, target):
            ...
            u= pred[:, 0:1]
            #time derivative
            u_t = torch.autograd.grad(u, xyt, torch.ones([u.shape[0],1]).to(device),retain_graph=True, create_graph=True)[0][:, 2:3]

            #first order derivative
            u_x = torch.autograd.grad(u, xyt, torch.ones([u.shape[0],1]).to(device),retain_graph=True, create_graph=True)[0][:, 0:1]
            u_y = torch.autograd.grad(u, xyt, torch.ones([u.shape[0],1]).to(device),retain_graph=True, create_graph=True)[0][:, 1:2]

            #second order derivative
            u_xx = torch.autograd.grad(u_x, xyt, torch.ones([u.shape[0],1]).to(device),retain_graph=True, create_graph=True)[0][:, 0:1]
            u_yy = torch.autograd.grad(u_y, xyt, torch.ones([u.shape[0],1]).to(device),retain_graph=True, create_graph=True)[0][:, 1:2]

            #PDE loss
            f0 = u_t - 1/((500*np.pi)**2)*u_xx - 1/(np.pi**2)*u_yy
    ...
    def closure(self):
        ...
        for i, (col_data, bc_data, ic_data) in enumerate(zip_longest(col_data, bc_data, ic_data)):
            ...
            #unpack data
            col_xyt = col_data
            bc_xyt, bc_u = bc_data[0], bc_data[1]
            ic_xyt, ic_u = ic_data[0], ic_data[1]

            #forward pass
            if col_xyt is not None:
                col_xyt = col_xyt.clone()
                col_xyt.requires_grad = True
                col_pred = self.model(col_xyt)
                col_loss = self.PDE_loss(col_pred, col_xyt)
                total_loss += col_loss
            if bc_xyt is not None:
                bc_pred = self.model(bc_xyt)
                bc_loss = self.BC_loss(bc_pred, bc_u)
                total_loss += bc_loss
            if ic_xyt is not None:
                ic_pred = self.model(ic_xyt)
                ic_loss = self.IC_loss(ic_pred, ic_u)
                total_loss += ic_loss
