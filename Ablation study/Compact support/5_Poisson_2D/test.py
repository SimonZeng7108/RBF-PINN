...
model.load_state_dict(torch.load("./logs/models/h2/epoch_34000.pt"))
...
plt.scatter(x, y, c=u, cmap='jet',vmin=0, vmax=1, marker = '.')
plt.axis('scaled')
plt.autoscale(tight=True)
plt.colorbar(shrink = 0.8, ticks=np.linspace(-1, 1, 5))


plt.savefig("./results/poisson_rbf.png", dpi= 300, bbox_inches='tight', pad_inches=0)
# plt.show()


absolute_e = np.abs(u - u_baseline)
plt.subplots(figsize=(5, 5))
plt.scatter(x, y, c=absolute_e, cmap='jet',vmin=0, vmax=0.0001, marker = '.')
plt.axis('scaled')
plt.autoscale(tight=True)
plt.colorbar(shrink = 0.8, ticks=np.linspace(0, 0.0001, 5))
plt.savefig("./results/poisson_rbf_ab.png", dpi= 300, bbox_inches='tight', pad_inches=0)
plt.show()
