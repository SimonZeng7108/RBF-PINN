...
class RBF_Layer(nn.Module):
    ...
    def __init__(self,ub, lb, in_features, out_features, basis_func, fixed_centroid = False, init_type = 'LHS'):
    ...
    def forward(self, input, h=4):
        size = (input.size(0), self.out_features, self.in_features)
        x = input.unsqueeze(1).expand(size)
        c = self.centres.unsqueeze(0).expand(size)
        
        r = (x - c).pow(2).sum(-1).pow(0.5)
        mask = torch.ones_like(r)
        mask[r>h] = 0
        r = r*mask

        distances = r / torch.exp(self.log_sigmas).unsqueeze(0)
