...
class Geo_Dataset:
    ...
    def __call__(self):
        ...
        ic_u = np.sin(np.pi * ic_x) + 1/2*np.sin(4*np.pi*ic_x)  #Dirichlet condition
        ic_dt = np.zeros((self.N_i, 1))                         #Neumann condition
        # plot_XYZ_2D(ic_x, ic_t, ic_u, show = True)
        # assert 1==2 
